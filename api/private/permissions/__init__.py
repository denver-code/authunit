...
async def get_permission(request: Request, user_id: str = None, service: str = None, value: str = None):
    ...
    if user_id and service and value:
        if user_id != jwt_token["user_id"]:
            await check_permission(request, "internal", "get_user_permission")
        else:
            await check_permission(request, "internal", "read_permission")
    ...
    permissions = await PermissionModel.find({"user_id": PydanticObjectId(jwt_token["user_id"])}).to_list()
    _p = {}
...
async def create_permission(request: Request, payload: NewPermission):
    await check_permission(request, "internal", "create_permission")
...
async def bulk_promote(request: Request, payload: BulkPromote):
    await check_permission(request, "internal", "bulk_promote")
